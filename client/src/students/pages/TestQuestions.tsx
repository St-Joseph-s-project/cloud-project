...
const TestQuestions: React.FC = () => {
    ...
    const questions = [
        ...
        { id: 'add-two-numbers', title: 'Add Two Numbers', difficulty: 'Medium', points: 200 },
        { id: 'longest-substring', title: 'Longest Substring Without Repeating Characters', difficulty: 'Medium', points: 200 },
    ];
    ...
    useEffect(() => {
        const handleVisibilityChange = () => {
            if (document.visibilityState === 'hidden' && isStarted) {
                ...
                setWarningMessage('Warning: Tab switching detected! This incident has been logged.');
                setShowWarning(true);
            }
        };
        ...
        const handleBlur = () => {
            if (isStarted) {
                setWarningCount(prev => prev + 1);
                setWarningMessage('Warning: Window focus lost! Please stay on this page.');
                setShowWarning(true);
            }
        };
    ...
    const handleBackClick = () => {
        if (isStarted) {
            setWarningMessage('Warning: You cannot leave the test while it is in progress!');
            setShowWarning(true);
        } else {
                ...
                <div className="bg-bg-card p-10 rounded-3xl shadow-2xl border border-border-gray max-w-lg w-full text-center space-y-6 animate-fade-in-up">
                    <div className="w-20 h-20 bg-primary-blue/10 rounded-2xl flex items-center justify-center mx-auto text-primary-blue">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-10 h-10">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m0-10.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.75c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.75A11.959 11.959 0 0112 2.714z" />
                        </svg>
                    </div>
                    <div>
                        <h2 className="text-2xl font-black text-text-main">Ready to Start?</h2>
                        <p className="text-text-body/60 mt-2">This test will be proctored. Fullscreen mode will be enabled automatically.</p>
                    </div>
                ...
                <div className="max-w-4xl w-full mx-auto space-y-6 animate-fade-in-up">
                    ...
                    <header className="bg-bg-card p-8 rounded-3xl shadow-sm border border-border-gray">
                        <div className="flex items-start justify-between">
                            <div>
                                ...
                                <p className="text-text-body/50 mt-1 font-medium">Please solve all challenges before the timer runs out.</p>
                            </div>
                    ...
                    <div className="space-y-4">
                        {questions.map((q, index) => (
                            <div key={q.id} className="bg-bg-card p-6 rounded-2xl shadow-sm border border-border-gray flex justify-between items-center hover:border-primary-blue/30 hover:scale-[1.01] transition-all group">
                                <div className="flex gap-4 items-center">
                                    <span className="w-10 h-10 rounded-xl bg-bg-main flex items-center justify-center font-black text-text-main border border-border-gray group-hover:bg-primary-blue group-hover:text-white group-hover:border-primary-blue transition-colors">{index + 1}</span>
                                    <div>
                                        <h3 className="font-bold text-text-main group-hover:text-primary-blue transition-colors">{q.title}</h3>
                                        <p className="text-[10px] text-text-body/50 font-bold uppercase tracking-tight">{q.difficulty} • {q.points} Points</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => navigate(`/student/${studentId}/${q.id}`, { state: { fromTest: slug } })}
            ...
            {showWarning && (
                <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-fade-in">
                    <div className="bg-bg-card p-8 rounded-3xl border border-red-200 shadow-2xl max-w-md w-full text-center space-y-6 animate-scale-in">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-red-600">Proctoring Alert</h2>
                            <p className="text-text-body/60 mt-2 font-medium">{warningMessage}</p>
                        </div>
