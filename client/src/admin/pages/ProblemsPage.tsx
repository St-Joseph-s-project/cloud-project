...
const ProblemsPage: React.FC = () => {
  ...
  const { items, filter, sortBy, loading, error } = useAppSelector(
  ...
  const handleSaveProblem = async (problem: Problem) => {
    try {
      if (drawerMode === "create") {
        ...
        await dispatch(
          updateProblemThunk({ id: problem.id, data: problem }),
        ).unwrap();
      }
      if (drawerMode === "create") {
        setDrawerOpen(false);
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case "Algorithms":
        return <PuzzlePieceIcon className="h-4 w-4 text-blue-600" />;
      case "Data Structures":
        return <DocumentTextIcon className="h-4 w-4 text-purple-600" />;
      case "SQL Database":
        return <CircleStackIcon className="h-4 w-4 text-green-600" />;
      default:
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold text-gray-800 mt-1">
                  {
                    safeItems.filter((p: Problem) => p.difficulty === "Easy")
                      .length
                  }
                </div>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold text-gray-800 mt-1">
                  {
                    safeItems.filter((p: Problem) => p.difficulty === "Medium")
                      .length
                  }
                </div>
              </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-lg font-bold text-gray-800 mt-1">
                  {
                    safeItems.filter((p: Problem) => p.difficulty === "Hard")
                      .length
                  }
                </div>
              </div>
                ...
                >
                  <div className="grid grid-cols-12 gap-4 px-6 py-4 items-center">
                    ...
                    <div className="col-span-2">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getDifficultyColor(problem.difficulty)}`}
                      >
                        {problem.difficulty}
                      </span>
                    </div>
        ...
        <div className="mt-6 flex items-center justify-between">
          <div className="text-sm text-gray-600">
            ...
            <span className="font-medium">
              1-{Math.min(filteredProblems.length, 10)}
            </span>{" "}
