...
export class ReactionService {
  ...
  async addBlogReaction(
    ...
    // Check if user already has ANY reaction on this blog
    // We ignore reaction_id here to find if they reacted at all
    ...
    if (existingReaction) {
      if (existingReaction.reaction_id === reaction_id) {
        ...
        const updatedReaction = await prisma.blog_reactions.update({
          where: { id: existingReaction.id },
          data: { reaction_id },
        });
        return {
          id: updatedReaction.id,
          reaction_id: updatedReaction.reaction_id,
